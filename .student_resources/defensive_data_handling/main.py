...
if __name__ == "__main__":
    ...
    print(
        f"Replace characters: {sanitiser.replace_characters('<script>alert("Hi")</script>')}"
    )
    print(
        f"Make <HTML> web safe: {sanitiser.make_web_safe('<script>alert("Hi")</script>')}"
    )
    print(
    ...
    # password = 123
    try:
        sanitiser.check_password(password)
    except TypeError:
